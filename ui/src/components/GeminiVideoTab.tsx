...
export const GeminiVideoTab: React.FC = () => {
  ...
  useEffect(() => {
    if (selectedNode && selectedNode.uploadedVideoFile && selectedNode.uploadedVideoSubfolder) {
  ...
  const handleTrimChange = (startTime: number, endTime: number) => {
    ...
    if (node) {
      ...
      const startTimeWidget = node.widgets?.find((w: any) => w.name === 'start_time')
      const maxDurationWidget = node.widgets?.find((w: any) => w.name === 'max_duration')
      ...
      {geminiNodes.length === 0 ? (
        <div className="no-nodes">
          <div className="empty-state">
            ...
            <p>Add a "Gemini Video Describe" node to your workflow and upload a video to start trimming.</p>
          </div>
        </div>
      ) : (
        <div className="nodes-container">
          <div className="node-selector">
            ...
            <select 
              ...
              onChange={(e) => {
                const node = geminiNodes.find(n => n.id.toString() === e.target.value)
            ...
            >
              ...
              {geminiNodes.map(node => (
                <option key={node.id} value={node.id}>
                  Node {node.id} {node.uploadedVideoFile ? `(${node.uploadedVideoFile})` : '(No video)'}
                </option>
              ))}
            </select>
          </div>
          ...
          {selectedNode && videoUrl ? (
            <div className="video-section">
              ...
              <VideoTrimmer
                ...
                initialEndTime={selectedNode.endTime || selectedNode.duration || 0}
              />
            </div>
          ) : selectedNode && !videoUrl ? (
            <div className="no-video">
              <h4>No Video Uploaded</h4>
              <p>This node doesn't have a video uploaded yet. Use the upload button on the node to select a video file.</p>
            </div>
          ) : null}
        </div>
      )}
