...
SK Custom Nodes is a ComfyUI extension that provides both Python custom nodes and React-based UI components. The project includes:
- Python custom nodes for Gemini AI video analysis 
- React/TypeScript UI extension with internationalization
...
### Bootstrap React UI Development
```bash
...
### React UI Testing
**WARNING**: Jest tests are currently broken due to ES module configuration issues.
```bash
cd ui-react_backup/ui
...
### React UI Linting (Partially Works)
```bash
cd ui-react_backup/ui
...
ls -la ../dist/example_ext/  # React build outputs
ls -la ../../web/js/  # JavaScript widgets
ls -la ../../__init__.py  # Python entry point
```
